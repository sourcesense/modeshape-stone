...
public class PaxConfigurations {
	...
	public static Option slingBasicConfiguration() {
		return composite(felix(),
				...
				slingCommonsLog(), jcr(), jackrabbit(), sling(),
				configurationAdmin(), osgi(), slf4j(),
	...
	public static Option lucene() {
		return composite(
				wrappedBundle(mavenBundle(LUCENE_GROUP, "lucene-core",
						LUCENE_VERSION)),
				wrappedBundle(mavenBundle(LUCENE_GROUP, "lucene-analyzers",
						LUCENE_VERSION)),
				wrappedBundle(mavenBundle(LUCENE_GROUP, "lucene-misc",
						LUCENE_VERSION)),
				wrappedBundle(mavenBundle(LUCENE_GROUP, "lucene-regex",
						LUCENE_VERSION)),
				wrappedBundle(mavenBundle(LUCENE_GROUP, "lucene-snowball",
						LUCENE_VERSION)));
	}

	public static Option joda() {
		return composite(mavenBundle(JODA_TIME_GROUP, "joda-time",
				JODA_TIME_VERSION));
	}

	public static Option jcip() {
		return composite(mavenBundle(JCIP_GROUP,
				"com.springsource.net.jcip.annotations", JCIP_VERSION));
	}

	public static Option modeshape() {
		return composite(
				mavenBundle(MODESHAPE_GROUP, "modeshape-common",
						MODESHAPE_VERSION),
				mavenBundle(MODESHAPE_GROUP, "modeshape-graph",
						MODESHAPE_VERSION),
				mavenBundle(MODESHAPE_GROUP, "modeshape-cnd", MODESHAPE_VERSION),
				mavenBundle(MODESHAPE_GROUP, "modeshape-repository",
						MODESHAPE_VERSION),
				mavenBundle(MODESHAPE_GROUP, "modeshape-search-lucene",
						MODESHAPE_VERSION),
				mavenBundle(MODESHAPE_GROUP, "modeshape-jcr-api",
						MODESHAPE_VERSION),
				mavenBundle(MODESHAPE_GROUP, "modeshape-jcr", MODESHAPE_VERSION));
	}

	public static Option modeshapeWeb() {
		return composite(mavenBundle(MODESHAPE_GROUP, "modeshape-web-jcr",
				MODESHAPE_VERSION));
	}

	public static Option configurationAdmin() {
		return composite(mavenBundle(FELIX_GROUP,
				"org.apache.felix.configadmin", CONFIGURATION_ADMIN_VERSION));
	}
	...
	public static Option stone_in_memory() {
		return composite(commonsIO(), commonsMath(), commonsCollections(),
				stone("in-memory"));
	}
	...
	public static Option osgi() {
		return composite(
				mavenBundle(OSGI_GROUP, "org.osgi.core", OSGI_VERSION),
				mavenBundle(OSGI_GROUP, "org.osgi.compendium", OSGI_VERSION));
	}

	public static Option jackrabbit() {
		return composite(
				mavenBundle(JACKRABBIT_GROUP, "jackrabbit-api",
						JACKRABBIT_VERSION),
				mavenBundle(JACKRABBIT_GROUP, "jackrabbit-jcr-commons",
						JACKRABBIT_VERSION),
				mavenBundle(JACKRABBIT_GROUP, "jackrabbit-jcr-rmi",
						JACKRABBIT_VERSION));
	}

	public static Option slf4j() {
		return composite(mavenBundle(SLF4J_GROUP, "slf4j-api", SLF4J_VERSION),
				mavenBundle(SLF4J_GROUP, "slf4j-simple", SLF4J_VERSION)
						.noStart());
	}

	public static Option sling() {
		return composite(
				mavenBundle(STONE_GROUP, "com.sourcesense.stone.jcr.base",
						STONE_VERSION),
				mavenBundle(SLING_GROUP, "org.apache.sling.jcr.api",
						SLING_VERSION));
	}
	...
	public static Option commonsCollections() {
		return wrappedBundle(mavenBundle("commons-collections",
				"commons-collections", "3.2.1"));
	}

	public static Option debug() {
		boolean debugIsEnabled = Boolean.getBoolean("debug");
		return composite(when(debugIsEnabled)
				.useOptions(
						vmOption("-Xrunjdwp:transport=dt_socket,server=y,suspend=y,address=5005")));
	}

	static Option slingCommonsLog() {
		return mavenBundle(SLING_GROUP, "org.apache.sling.commons.log",
				SLING_VERSION);
	}
	...
	static Option externalDependencies() {
		return mavenBundle(STONE_GROUP,
				"com.sourcesense.stone.external.dependencies", STONE_VERSION);
	}
	...
	static Option hibernate() {
		return composite(
				...
				wrappedBundle(mavenBundle("org.hibernate",
						"hibernate-entitymanager", "3.6.1.Final")),
				wrappedBundle(mavenBundle("org.hibernate",
						"hibernate-annotations", "3.5.2-Final")),
	...
	static Option modeshapeJPA() {
		return composite(mavenBundle(MODESHAPE_GROUP,
				"modeshape-connector-store-jpa", MODESHAPE_VERSION));
	}

	static Option commonsMath() {
		return composite(mavenBundle(APACHE_COMMONS_GROUP, "commons-math",
				"2.1"));
	}
	...
	private static MavenArtifactUrlReference createStoneServerArtifactReference(
			String classifier) {
		...
		mavenArtifactUrlReference
				.artifactId("com.sourcesense.stone.jcr.modeshape.server");
